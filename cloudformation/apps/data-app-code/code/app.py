...
title("Documentations",25,"center","white")
...
intro_text = '''
This app will collect your preferences of Anime genres and will suggest the most well rated to you,
based on data analysis made using AWS Data Wrangler to explore a dataset with more than 20k Anime titles 
...
# with journey_col3:
#     start_button = st.button('Hi, I\'m a Button',help='Hit me!')
...
with st.spinner('Wait a moment...'):
    ...
    def get_s3_parquet(filename):
        return wr.s3.read_parquet(f's3://{DATASETS_BUCKET_NAME}/{filename}')
...
with st.beta_expander('Anime Preference Form'):
    with st.form("Anime Preference"):
        ...
        recommendation_based_on = st.selectbox(label='Attribute',options=['Rank','Popularity'])
...
user_friendly_selected_genres = ''
for i in range(len(selected_genres)):
    if i == 0:
        user_friendly_selected_genres+=selected_genres[i]
    elif i == len(selected_genres)-1:
        user_friendly_selected_genres = user_friendly_selected_genres + ' and ' + selected_genres[i]
    else:
        user_friendly_selected_genres = user_friendly_selected_genres + ', ' + selected_genres[i]

thank_you_text = f'''
So {user_name}, I can see you like {user_friendly_selected_genres} animes, right? Thank you for sharing!
'''

if submitted and len(selected_genres) >0 and len(selected_genres) <= 5 and user_name != '':
    title(thank_you_text,25,'center','white')
...
with st.beta_expander('Details'):
    ...
    session_break(1)
    st.markdown(f"Visit title\'s [MyAnimeList.Net page](https://myanimelist.net/anime/{top_n_rank_list[0][1]}/) to check more info about it :)")
...
with st.beta_expander('Details'):
    ...
    session_break(1)
    st.markdown(f"Visit title\'s [MyAnimeList.Net page](https://myanimelist.net/anime/{top_n_pop_list[0][1]}/) to check more info about it :)")
...
title('IT\'S TIME!',50,'center','red')
recommend_header_col1, recommend_header_col2, recommend_header_col3 = st.beta_columns((7,4,4))
with recommend_header_col2:
        st.markdown('# :clock130:')
title('This is What We Recommend To You Today',40,'center','white')
...
st.markdown('### If you enjoyed this data app, please leave your ⭐ in the GitHub repository and follow me in the social medias ❤️')

social_medias_col1, social_medias_col2, social_medias_col3 = st.beta_columns((1,1,3))

with social_medias_col1:
